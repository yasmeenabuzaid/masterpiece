...
<div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="title-1">sub categories</h2>
        <a href="{{ route('subcategories.create') }}">
            <button type="button" class="btn btn-primary">
                <i class="zmdi zmdi-plus"></i> Add New sub categorie
            </button>
        </a>
    </div>
                ...
                <table class="table table-bordered bg-white">
                    ...
                    @foreach($subcategories as $subcategorie)
                        <tr>
                            ...
                            <th scope="row">{{ $subcategorie->name }}</th>
                            <th scope="row">{{ $subcategorie->description }}</th>
                            <th scope="row">{{ $subcategorie->subcat->name }}</th>
                            <td>{{ $subcategorie->created_at->format('Y-m-d') }}</td>
                             <th scope="row">
                                <a href="{{ route('subcategories.edit', $subcategorie->id) }}">
                                    <button type="button" class="btn btn-secondary"><i class="fa-solid fa-pen-to-square"></i></button>
                      
                                </a>
                               
                                <button type="button" class="btn btn-danger" onclick="confirmDeletion(event, '{{ route('subcategories.destroy', $subcategorie->id) }}')"><i class="fa-solid fa-trash"></i></button>
                            </th>
                        </tr>
                    @endforeach
                    </tbody>
